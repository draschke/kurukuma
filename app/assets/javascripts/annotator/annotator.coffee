...
class Annotator extends Delegator
  ...
  showEditor: (annotation, location) =>
    ...
    if @plugins.Twitter && typeof annotation.text is 'undefined'
      annotation.text = @plugins.Twitter.initTextFormat(@selectedText)
      
    @editor.load(annotation)
    this.publish('annotationEditorShown', [@editor, annotation])
    elem = $('#annotator-field-0')[0]
